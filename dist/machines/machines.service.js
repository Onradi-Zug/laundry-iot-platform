...
let MachinesService = class MachinesService {
    ...
    findByLaundry(laundryId) {
        return this.repo.find({
            where: { laundry: { id: laundryId } },
            relations: ['laundry', 'tenant']
        });
    }
    findById(id) {
    ...
    updateStatus(id, status) {
        return this.repo.update(id, { status, updatedAt: new Date() });
    }
    findStuckMachines(seconds) {
        return this.repo
            .createQueryBuilder('machine')
            .where('machine.status = :status', { status: 'running' })
            .andWhere('machine.updatedAt < NOW() - INTERVAL \'' + seconds + ' seconds\'')
            .getMany();
    }
};
